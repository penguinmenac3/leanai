"""doc
# deeptech.core.cli_tools

> The command line interface for project management with deeptech.

You can create a new project with the following command:
```bash
deeptech_tools --new_project --name=<name>
```

Also you can add run configurations for vscode for a project with this command:
```bash
deeptech_tools --vscode_run_config --name=<name> --module=<module> <arg1> <arg2> ...
```
"""
import sys
import os
...
import argparse
import shutil
TEMPLATE_CLONE_URL = "https://github.com/penguinmenac3/deeptech-template.git"
...
def new_project(args):
    ...
    vscode_run_config(["Train", "{}.config.train".format(args.name), "--mode=train", "--input=input", "--output=results"])
    vscode_run_config(["Test", "{}.config.test".format(args.name), "--mode=test", "--input=input", "--output=results"])
...
def vscode_run_config(args):
    ...
    parser.add_argument('module', type=str, help="The name of the module that should be executed, e.g. 'deeptech.core.cli'")
    args, run_args = parser.parse_known_args(args)
