...
from deeptech.core import RunOnlyOnce, logging
from deeptech.core.logging import debug
from deeptech.model.layers import Gather
...
from deeptech.training.losses.iou_box_2d import similarity_iou_2d
from deeptech.training.losses.masking import NaNMaskedLoss, NegMaskedLoss
...
class DetectionLoss(Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        tensorboard.log_scalar("loss/class({}-{})".format(self.pred_class_ids, self.target_class_ids), class_loss)
        tensorboard.log_scalar("loss/center({}-{})".format(self.pred_boxes, self.target_boxes), center_loss)
        tensorboard.log_scalar("loss/size({}-{})".format(self.pred_boxes, self.target_boxes), size_loss)
